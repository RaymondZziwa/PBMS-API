...
export class AuthMicroserviceService {
  ...
  validatedAccessToken(data: validateTokenDto): boolean {
    return this.jwtService.verifyTokens(data.token);
  }
  //create user
  async createUser(data: createUserDto) {
    try {
      ...
      const userPassword = bcrypt.hash(data.password, 12);
      const newUserCreationQuery = Prisma.sql`INSERT INTO USER (first_name, last_name, gender, nin_number, email, password, branch, department, role, salary, dob, contact1, contact2) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
      const values = [
        data.first_name,
        data.last_name,
        data.gender,
      ...
      await this.prismaService.$queryRaw(newUserCreationQuery, values);
      return {
        statusCode: 201,
        message:
  ...
  async getUser(data: getUserDto) {
    try {
      ...
      const user = await this.prismaService
        .$queryRaw`SELECT * FROM user WHERE user_id = ${user_id}`;
      if (user) {
        return {
          statusCode: 200,
  ...
  async renewAccessToken(data: renewAccessTokenDto) {
    try {
      ...
      const userData = await this.prismaService
        .$queryRaw`SELECT user_id, email, branch, department, role WHERE user_id = ${data.user_id}`;
      const isRefreshTokenValid: boolean =
        this.jwtService.verifyTokens(refreshToken);
      if (isRefreshTokenValid) {
        this.jwtService.generateAccessToken(userData[0]);
      }
    } catch (error) {
